...
import { getCurrentRate, getOfficialQuotes, listRateHistory, setRate } from "../controllers/exchangeRate.controller";

export const exchangeRateRouter = Router();

exchangeRateRouter.use(requireAuth);
exchangeRateRouter.get("/current", getCurrentRate);
exchangeRateRouter.get("/quotes", getOfficialQuotes);
